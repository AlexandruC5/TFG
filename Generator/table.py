...
for x in range(5):
    ...
    top = box(pos=vector(0+increment, 0, 0),
              axis=vector(0, 0, 0),
    ...
    pata1 = cylinder(pos=vector(8.5+increment, 0, -4.5),
                     axis=vector(0, -5, 0), radius=0.5, color=color.red)

    # pata 2= izquierda abajo
    pata2 = cylinder(pos=vector(-8.5+increment, 0, 4.5), axis=vector(0, -
                                                                     5, 0), radius=0.5, color=color.yellow)

    # pata3 = derecha abajo
    pata3 = cylinder(pos=vector(8.5+increment, 0, 4.5), axis=vector(
        0, -5, 0), radius=0.5, color=color.green)

    # pata 4= pata izquierda arriba
    pata4 = cylinder(pos=vector(-8.5+increment, 0, -4.5),
